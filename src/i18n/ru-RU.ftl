...
settings-audio-boost-desc = Усиливает декодированный поток вне единичного уровня.
settings-color-depth-desc = При 32-бит исчезает эффект постеризации в небе и тёмных участках. 16-бит, в свою очередь, требует меньше памяти. Применяется при следующем запуске.
...
settings-stick-zones-desc = L3/R3, из нижних углов сенсорной панели спереди (экран).
...
settings-trigger-swap-desc = Нажимает L2/R2 при нажатии L1/R1 и наоборот, для игр, в раскладке которых основное действие привязано к триггерам.
...
server-picker-powered-by = Данные очереди на основе PrintedWaste
server-picker-hint = Вверх/Вниз чтобы выбрать · X чтобы запустить · O чтобы отменить

# Power / link health
status-battery-low = Низкий заряд баратери ({ $percent }%) - сессия остановится прежде чем батарея разрядится.
status-battery-critical = Критический заряд батареи - сессия принудительно остановлена для избежания ошибок завершения.
